...
public class ProgressWheel extends View {
  ...
  private void parseAttributes(TypedArray a) {
    ...
    circleRadius = (int) a.getDimension(R.styleable.ProgressWheel_wheel_circleRadius, circleRadius);

    fillRadius = a.getBoolean(R.styleable.ProgressWheel_wheel_fillRadius, false);

    barWidth = (int) a.getDimension(R.styleable.ProgressWheel_wheel_barWidth, barWidth);

    rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_wheel_rimWidth, rimWidth);

    float baseSpinSpeed =
        a.getFloat(R.styleable.ProgressWheel_wheel_spinSpeed, spinSpeed / 360.0f);
    spinSpeed = baseSpinSpeed * 360;

    barSpinCycleTime =
        a.getInt(R.styleable.ProgressWheel_wheel_barSpinCycleTime, (int) barSpinCycleTime);

    barColor = a.getColor(R.styleable.ProgressWheel_wheel_barColor, barColor);

    rimColor = a.getColor(R.styleable.ProgressWheel_wheel_rimColor, rimColor);

    linearProgress = a.getBoolean(R.styleable.ProgressWheel_wheel_linearProgress, false);

    if (a.getBoolean(R.styleable.ProgressWheel_wheel_progressIndeterminate, false)) {
      spin();
    }
